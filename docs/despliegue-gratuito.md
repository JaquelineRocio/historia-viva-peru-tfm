...
`MODAL_PROXY_TOKEN_SECRET`. El secreto de Modal contiene además
`ML_SUPADATA_API_KEY` para el fallback de subtítulos. Consulte
`docs/modal-paso-a-paso.md`.
...
## 4. Supabase Storage — archivos

1. Cree un proyecto gratuito y un bucket privado `historia-viva-tfm`.
2. En la configuración de Storage, cree credenciales S3 con acceso al proyecto.
3. Anote el endpoint S3, access key y secret.
...
$env:S3_BUCKET='historia-viva-tfm'
$env:S3_REGION='us-west-2'
$env:S3_ACCESS_KEY_ID='...'
$env:S3_SECRET_ACCESS_KEY='...'
node apps/api/scripts/upload-demo-s3.mjs artifacts/demo/files
```
...
Supabase Storage expone una interfaz compatible con S3; por eso las variables de
producción se llaman `S3_*`.
...
2. Complete todas las variables con `sync: false`:
   ...
   - credenciales S3 de Supabase Storage.
...
Después incorpore la URL del vídeo al README y pruebe todos los enlaces desde una
ventana privada. Los niveles gratuitos pueden tardar al despertar; eso debe
indicarse en la defensa.

Referencias: [Render Free](https://render.com/docs/free),
[Neon](https://neon.com/pricing), [Supabase Storage](https://supabase.com/docs/guides/storage),
