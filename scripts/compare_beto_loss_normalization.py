"""One predeclared effective-batch loss intervention on two existing TRAIN holdouts."""
...
ART = ROOT / 'artifacts/experiments/beto-loss-normalization-v1'
OUT = ROOT / 'outputs/beto-loss-normalization-v1'
...
def decide(folds):
    ...
    return dict(status=status, deltas=deltas, selected_model=None,
                next_action='Stop this intervention; no automatic additional loss variants, rates, epochs or promotion.')
...
def freeze():
    ...
    control = read(ROOT / 'artifacts/experiments/beto-learning-rate-v1/protocol.json')
    guards = {g['path']: g for g in control['input_guards']}
    review = read(ROOT / 'artifacts/reviews/independent-review-v1/report.json')
    guards.update({g['path']: g for g in review['input_guards']})
    for g in guards.values():
        assert sha(ROOT / g['path']) == g['sha256'], g['path']
    paths = [PARENT, ROOT / 'scripts/compare_beto_loss_normalization.py',
    ...
    paths += [ROOT / 'scripts/beto_effective_batch_loss.py', ROOT / 'scripts/check_beto_effective_batch_loss.py', ROOT / 'outputs/beto-loss-normalization-v1-preflight.json', ROOT / 'artifacts/datasets/evaluation-pilot-v2.json']
    paths += list((ROOT / 'artifacts/reviews/independent-review-v1').glob('*.json'))
    ...
    loss_check = read(ROOT / 'outputs/beto-loss-normalization-v1-preflight.json')
    assert loss_check['status'] == 'passed'
    assert loss_check['trainer_sha256'] == sha(ROOT / 'scripts/beto_effective_batch_loss.py')
    assert loss_check['checker_sha256'] == sha(ROOT / 'scripts/check_beto_effective_batch_loss.py')
    candidate_params = dict(parent['params'])
    ...
    protocol = dict(status='frozen_before_training', created_at_utc=now(), parent=guard(PARENT),
        question='Does effective-group weighted CE normalization improve fit and transfer at fixed epoch2?',
        rationale='Microbatch weighted means do not equal effective-group weighted mean. Correct that objective mismatch, verified by loss and gradient fixtures. Causal relevance remains unknown.',
        baseline_params=parent['params'], candidate_params=candidate_params, changed_parameter='loss_normalization',
        ...
        invariants='Same offline base/revision, seed42, labels/order/texts, batch2 accumulation8, inverse weights, lr2e-5, AdamW decay.01, clipping1, AMP, checkpointing, 384 tokens, scheduler horizon3 and warmup. Separate trainer with group denominator; stop at epoch2.',
        counters_policy='Record actual AMP skips and optimizer updates; numerical trajectories can differ as a consequence of loss normalization. Do not claim equal actual updates without checking.',
        stop='Exactly two fresh base trajectories, one per fold, one snapshot each. No retraining baseline, automatic retries, third epoch, additional variants or model promotion.',
        limits=['Reused TRAIN holdouts, not independent evaluation.', 'One normalization change and one seed do not prove a unique cause or general BETO capacity.',
                'Corpus, guide and three proposed corrections unchanged; no validation/test/external31/new-pilot22 predictions.'])
    protocol['content_sha256'] = fingerprint(protocol)
    ART.mkdir(parents=True)
    write(ART / 'protocol.json', protocol)
    print('Frozen: effective-group weighted CE, lr2e-5, epoch2, two folds; 5 decision checks passed.', flush=True)
...
def inputs():
    ...
    assert p['candidate_params'] == dict(parent['params'])
...
def verify():
    ...
    for fold in p['folds']:
        ...
        for key in ('base_model', 'resolved_revision', 'class_counts', 'class_weights', 'schedule_epochs',
                    ...
                    'max_len', 'optimizer', 'weight_decay', 'class_weighting',
        ...
        assert old['loss_normalization'] == 'weighted_CE_mean_per_microbatch_then_divide_by_actual_accumulation_group_size'
    ...
    write(ART / 'verification.json', dict(status='passed', metric_summaries_recomputed=checks,
        ...
        single_config_change='microbatch means -> effective-group weighted mean', baseline_epoch=2, candidate_epoch=2,
