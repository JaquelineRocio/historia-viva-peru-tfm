...
def inputs():
    ...
    for path,h in p['input_hashes'].items():
        target=ROOT/path
        if path=='scripts/run_beto_first_training_v2.py':
            target=ROOT/'artifacts/beto-v2/stability/runner-seed42-original.py'
        if filehash(target)!=h:raise ValueError('Changed frozen input: '+path)
...
def run(seed=42):
    ...
    p,byid=inputs()
    if seed in (43,44):
        return stability_run(seed,p,byid)
    for fold in p['folds']:
...
def verify_result(dest,protocol,val):
    result=read(dest/'result.json')
    if result['status']!='complete' or result['protocol_hash']!=fingerprint(protocol):
        raise ValueError('Completed execution identity mismatch: '+str(dest))
    expected=[{k:r[k] for k in ('segment_id','family_id','role','text_sha256')} | {'truth':r['label']} for r in val]
    actual=[{k:v for k,v in r.items() if k!='predicted'} for r in result['predictions']]
    if expected!=actual:raise ValueError('Completed validation identity mismatch')
    checkpoint=dest/f"checkpoint-{result['best_epoch']}"
    for name,h in result['checkpoint_hashes'].items():
        if filehash(checkpoint/name)!=h:raise ValueError('Checkpoint hash mismatch')
    if subsets(val,[r['predicted'] for r in result['predictions']])!=result['metrics']:
        raise ValueError('Saved metrics mismatch')
    return result

def stability_run(seed,p,byid):
    if seed not in (43,44):raise ValueError('Only two additional seeds authorized')
    art=ROOT/'artifacts/beto-v2/stability'
    manifest={'original_protocol_hash':fingerprint(p),'original_protocol_file_sha256':filehash(ART/'protocol.json'),
              'runner_sha256':filehash(ROOT/'artifacts/beto-v2/stability/runner-stability-original.py'),'seeds':[43,44],'maximum_new_runs':12,
              'shuffle':'seed+epoch','initialization':'seed before base model construction',
              'baseline_hashes':{f['id']+'-'+c:filehash(OUT/'runs'/(f['id']+'-'+c)/'baselines.json') for f in p['folds'] for c in ('A','B')}}
    if (art/'protocol.json').exists():
        if read(art/'protocol.json')!=manifest:raise ValueError('Stability manifest changed')
    else:save(art/'protocol.json',manifest)
    for fold in p['folds']:
        val=[byid[i] for i in fold['validation']]
        for condition in ('A','B'):
            verify_result(OUT/'runs'/(fold['id']+'-'+condition),p,val)
            train=[byid[i] for i in fold['train_'+condition]];validate(train,val)
            execution={'seed':seed,'fold':fold['id'],'condition':condition,'train_rows_hash':fingerprint(train),
                       'validation_rows_hash':fingerprint(val),'stability_manifest_hash':fingerprint(manifest)}
            config={**p,'execution':execution}
            dest=ROOT/'outputs/beto-v2/stability'/f'seed-{seed}'/(fold['id']+'-'+condition)
            if (dest/'config.json').exists():
                if read(dest/'config.json')!=config:raise ValueError('Execution configuration changed')
            else:save(dest/'config.json',config)
            if (dest/'result.json').exists():
                verify_result(dest,config,val);print('Reusing verified '+str(dest),flush=True)
            else:train_run(train,val,dest,config)

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('command',choices=['run']);parser.add_argument('--seed',type=int,choices=[42,43,44],default=42)
    parser.add_argument('--weight-source',choices=['condition_train','train_A'],default='condition_train')
    args=parser.parse_args()
    if args.weight_source=='train_A':
        from run_beto_class_weights_v2 import run_weights
        run_weights(args.seed)
    else:run(args.seed)
